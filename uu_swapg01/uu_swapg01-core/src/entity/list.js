//@@viewOn:imports
import { createComponent, createVisualComponent, Utils, useState, useRoute } from "uu5g05";
...
const SERIE_LIST = (setRoute) => [
  ...
  // <Uu5Elements.Button icon="uugds-open-in-new"
  //                     onClick={() => setRoute('', { data: { speciesName: "jdem spat!!" } })}
  // />
  {
    value: "touchButton",
    label: "Detail",
    dataItem: (data) => (
      <Uu5Elements.TouchButton
        ...
        onClick={() => setRoute("writerGate/entityDetail", data)}
        size="s"
...
const List = createVisualComponent({
  ...
  render(props) {
    ...
    const [serieList, setSerieList] = useState(SERIE_LIST(setRoute));
    //@@viewOff:private
